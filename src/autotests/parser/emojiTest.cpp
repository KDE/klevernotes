...
{
    m_md4qtParser.addTextPlugin(256, EmojiExtension::emojiHelperFunc, true, {});
...
void EmojiTest::simpleEmojiMiddle()
{
    ...
    const auto item2 = std::static_pointer_cast<EmojiExtension::EmojiItem>(paragraph->getItemAt(1));
...
void EmojiTest::simpleEmojiStart()
{
    ...
    const auto item1 = std::static_pointer_cast<EmojiExtension::EmojiItem>(paragraph->getItemAt(0));
    QCOMPARE_EQ(item1->startColumn(), 0);
    QCOMPARE_EQ(item1->endColumn(), 6);
    QCOMPARE_EQ(item1->startLine(), 0);
    QCOMPARE_EQ(item1->endLine(), 0);
